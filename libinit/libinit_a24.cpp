...
void vendor_load_properties()
{
    ...
    if (bootloader_name != "")
    {
        ...
        if (! props_loaded)
        {
            LOG(WARNING) << LOGTAG_LIB << ": information about B/E/M variants was not found in the bootloader version! The device could not be detected. Properties belonging to the A245F model code will be used.";
            load_properties("A245F");
        }
    }
    else
    {
        LOG(WARNING) << LOGTAG_LIB << ": failed to get " << BOOTLOADER_PROPERTY << " property content. Properties belonging to the A245F model code will be used.";
        load_properties("A245F");
    }
